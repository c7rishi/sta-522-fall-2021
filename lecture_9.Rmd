...
$L(\theta \mid \vec x)$ is an increasing function in $\theta$ for $\theta \in (-\infty, x_{(1)}]$. So unrestricted MLE is $\hat \theta = x_{(1)}$ so that $\ds \sup_{\theta \in \Theta} = L(x_{(1)} \mid \vec x) = e^{-n \left(\bar x-x_{(1)}\right)}$. 
...
$$
$$
\beta_2(\theta) = P_\theta(X \in R_2) = \sum_{j=3}^5 P_\theta(X = 5) = \sum_{j=3}^5 \binom{5}{j} \theta^j (1-\theta)^{5-j}
$$
