...
L(k \mid \underline x, p) = \prod_{i=1}^n \binom{k}{x_i} p^{x_i} (1-p)^{x_i}
...
$$
f_{Y \mid p}(y \mid p) = \frac{f_{Y, P}(y, p)}{f_P(p)} = \frac{\Gamma(n+\alpha+\beta)}{\Gamma(y + \alpha) \Gamma(n-y+\beta)} \ p^{y+\alpha-1} (1-p)^{n-y + \beta -1}
$$
