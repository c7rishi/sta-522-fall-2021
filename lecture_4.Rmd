...
$$
\begin{aligned}
F_{X_1/X_n, \dots, X_{n-1}/X_n}(y_1, \dots, y_{n-1} \mid \theta) 
...
- Consider the Uniform($\theta - \frac12, \theta+\frac12$) example. Here $\theta \implies \left(R = X_{(n)} - X_{(1)}, M = (X_{(1)} + X_{(n)})/2\right)$ is  minimal sufficient for $\theta$,  but $R$ is ancillary. 
...
$$
g(T(\vec X)) = \frac{n}{n+1} T_1(\vec X)^2 -  T_2(\vec)
$$
