...
- Unbiased is a good property for an estimator to have, but it can be misleading.
...
$$
\frac{\left[\frac{d}{d\beta}\E_{\beta}\left[W(\mathbf{X})\right]\right]^2}{n \E_{\beta}\left[\left[\frac{\partial}{\partial\beta}\log{f(X_1 \mid \beta)}\right]^2\right]} = \frac{1}{\alpha/\beta^2} = \frac{\beta^2}{\alpha} = \var_\beta(W(\vec X))
$$
