...
**Part (b):**  \underline{Sufficiency:} The pmf of $X$ is
$$
...
\underline{Completeness:} As suggested in the hint, we first find the pmf of $Y = |X|$. We note that the support of $Y$ is $\{0, 1\}$. Clearly, $P(Y = 0) = P(X = 0) = \left(\frac{\theta}{2}\right)^{0} (1-\theta)^{1-0} = 1-\theta$, and 
$$
\begin{aligned}
P(Y = 1) &= P(X = 1) + P(X = -1) \\
...
**Part (b):** As suggested in the hint consider a sequence $X_n$ where $X_n = X$, and $Y = 1-X$, where $X \sim \on{Binomial}(1, 0.5)$. Then $X$ and $Y = 1-X$. Then $X_n \xrightarrow{d} X$ (trivially; all have the same distribution) which means $X_n \xrightarrow{d} Y$ as $X$ and $Y$ have the same distribution. 

However, for any $0 < \epsilon < 1$, 
$$
\begin{aligned}
P(|X_n - Y| \geq \epsilon) = P(|X - 1 + X| \geq \epsilon) 
